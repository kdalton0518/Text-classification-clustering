...
def generate_topics_per_folder(matrix_object):
    ...
    for i in range(len(my_process_objects)):
        ...
        if my_process_objects[i].file_basename.startswith("C1"):
            ...
            for j in range(len(matrix_object.keywords_concepts)):
                ...
                c1_aggregate_vector[j] = float(
                    c1_aggregate_vector[j] + this_cell_value)
            ...
            for j in range(len(matrix_object.keywords_concepts)):
                ...
                c4_aggregate_vector[j] = float(
                    c4_aggregate_vector[j] + this_cell_value)
            ...
            for j in range(len(matrix_object.keywords_concepts)):
                ...
                c7_aggregate_vector[j] = float(
                    c7_aggregate_vector[j] + this_cell_value)
    ...
    # process the results
    c1_results = sort_topics(
        matrix_object.keywords_concepts, c1_aggregate_vector)
    c4_results = sort_topics(
        matrix_object.keywords_concepts, c4_aggregate_vector)
    c7_results = sort_topics(
        matrix_object.keywords_concepts, c7_aggregate_vector)
...
def cosine_similarity(vector_1, vector_2):
    ...
    cosine_similarity = dot_prod(
        vector_1, vector_2) / (mag(vector_1) * mag(vector_2) + 0.000000001)
...
def euclidean_distance(vector_1, vector_2):
    ...
    euclidean_distance = sqrt(
        sum((e1-e2)**2 for e1, e2 in zip(vector_1, vector_2)))
...
class KMeans():
        ...
        for document_index in range(len(self.documents)):
            ...
            for centroid_index in range(len(self.centroids)):
                ...
                cosine_sim_score = cosine_similarity(
                    document_vector, self.centroids[centroid_index])
            ...
            logger.debug("in the end, document index %s is most similar to the %s'th centroid." % (
    ...
    def update_centroids(self):
        ...
        if exactly_equal(new_centroids, self.centroids):
            logger.debug(
                "You have met the stopping criterion. Returning to caller ... ")
            return False
...
class Visualize():
    ...
    def plot_actual(self):
        ...
        for i in range(len(files)):
            ...
            if i in clusters[0]:
                ax.scatter(x_vals[i], y_vals[i], color="teal",
                           label="police force & air china", alpha=0.3)

            elif i in clusters[1]:
                ax.scatter(x_vals[i], y_vals[i], color="magenta",
                           label="mouth disease & healthcare", alpha=0.3)

            elif i in clusters[2]:
                ax.scatter(x_vals[i], y_vals[i], color="orange",
                           label="mortgages & banks", alpha=0.3)
...
class ConfusionMatrix():
    ...
    def generate_confision_matrix(self):
        ...
        self.confusion_matrix = confusion_matrix(
            self.actual_vector, self.predicted_vector)
    ...
    def consider_precision_recall(self):
        ...
        self.precision = np.average(
            true_positive / (true_positive + false_positive))
        self.recall = np.average(
